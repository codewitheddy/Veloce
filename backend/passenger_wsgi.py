...
# 1. Ensure backend directory and current directory are in sys.path
app_dir = os.path.dirname(os.path.abspath(__file__))
if app_dir not in sys.path:
    sys.path.insert(0, app_dir)

# 2. Load .env file automatically if python-dotenv is available
...
try:
    ...
    # Capture and log startup traceback for cPanel troubleshooting
    err_msg = traceback.format_exc()
    log_file = os.path.join(app_dir, 'passenger_wsgi_error.log')
    try:
    ...
    # Provide a helpful error handler for Passenger in case of initial config issue
