...
export default function BestSellingByCategory({
  ...
  const categorizedBestSellers = categories.map(category => {
    ...
    const displayCategorySales = topThree.reduce((sum, p) => {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between border-b border-indigo-100 pb-5 mb-10">
          <div>
            ...
            <p className="mt-1.5 text-xs text-gray-400 font-extralight leading-relaxed">
              Analyzing real-time order history to determine the top 3 high-performance objects in each category.
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="space-y-12">
          {categorizedBestSellers.map(({ category, products: categoryProducts }) => (
            <div key={category} className="bg-white rounded-2xl border border-indigo-50/60 p-6 sm:p-8 shadow-[0_4px_20px_rgba(37,44,139,0.02)]">
              {/* Category Sub-Header */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categoryProducts.map((product, index) => {
                  ...
                  const rankStyles = [
                    { bg: 'bg-amber-500', text: 'text-white', label: '#1 Best Seller' },
                    { bg: 'bg-slate-700', text: 'text-white', label: '#2 Top Contender' },
                    { bg: 'bg-indigo-600', text: 'text-white', label: '#3 Hot Choice' }
                  ];
                  const currentRank = rankStyles[index] || { bg: 'bg-indigo-100', text: 'text-indigo-800', label: `Rank #${index + 1}` };

                  return (
                    <div
                      key={product.id}
                      onClick={() => onProductClick(product)}
                      className="group relative flex flex-col justify-between rounded-xl border border-indigo-50 bg-[#fbfcfd]/40 p-4 cursor-pointer transition-all duration-300 hover:shadow-[0_8px_24px_rgba(37,44,139,0.04)] hover:border-indigo-200/60 hover:bg-white"
                    >
                      <div>
                        {/* Image Container with Rank Badge */}
                        <div className="relative h-44 w-full overflow-hidden rounded-lg bg-gray-50 border border-indigo-50/30">
                          <img
                          ...
                          {/* Rank Badge */}
                          <div className={`absolute top-2.5 left-2.5 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider shadow-xs ${currentRank.bg} ${currentRank.text} flex items-center gap-1 font-mono`}>
                          ...
                          {/* Sale Discount Tag */}
                          {isSale && (
                            <div className="absolute top-2.5 right-2.5 px-1.5 py-0.5 rounded bg-amber-500 text-white text-[9px] font-mono font-black uppercase shadow-xs">
                              -{discountPercent}% OFF
                            </div>
                          )}
                        </div>
                        ...
                        <div className="mt-3.5">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold uppercase font-mono tracking-wider text-indigo-500">
                              {product.category}
                            </span>
                            <div className="flex items-center text-amber-500 text-xs font-bold">
                              <Star className="h-2.5 w-2.5 fill-current" />
                              <span className="ml-1 text-[10px]">{product.rating.toFixed(1)}</span>
                            </div>
                          </div>

                          <h4 className="font-display font-semibold text-xs text-gray-900 mt-1 uppercase tracking-tight line-clamp-1 group-hover:text-indigo-600 transition-colors">
                            {product.name}
                          </h4>
                          <p className="mt-1 text-xs text-gray-500 font-extralight line-clamp-2 leading-relaxed">
                            {product.description}
                          </p>
                          ...
                          <div className="mt-3 flex items-center gap-1">
                            ...
                            <span className="text-[10px] font-semibold text-gray-700 ml-1">{product.rating}</span>
                      ...
                      <div className="mt-4 pt-3 border-t border-indigo-50/50 flex items-center justify-between">
                        ...
                        <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-indigo-600 group-hover:text-indigo-850 transition-colors">
                          Acquire Object <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" />
                        </span>
                      </div>
                    </div>
                  );
